...
import { Plus, Package, Search, Trash2, PackageX } from 'lucide-react';
...
export const WorkOrderPartsDialog: React.FC<WorkOrderPartsDialogProps> = ({
  ...
  // Helper to safely get inventory item details regardless of structure (array or object)
          ...
          <div className="flex-1 overflow-auto p-4">
                      ...
                      >
                        ...
                        <div className="text-right">
                          <p className={`text-sm font-medium ${isOutOfStock ? 'text-destructive' : ''}`}>
                            {formatQuantityWithUnit(qty, item.unit_of_measure || (item as any).unitOfMeasure)}
                          </p>
                          <p className="text-xs text-muted-foreground">UGX {((item as any).unitPrice ?? item.unit_price ?? 0).toLocaleString()}</p>
                        </div>
                      </button>
                    );
                  })
            ...
            <TabsContent value="used" className="mt-0 space-y-3">
              {isLoadingParts ? (
                ...
                <>
                  {usedParts.map(part => {
                    const item = getItemDetails(part.inventory_items);
                    return (
                      <div
                        key={part.id}
                      ...
                      >
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground">
                            {item?.sku || 'No SKU'}
                            {item?.model && ` • ${item.model}`} • Qty: {part.quantity_used}
                          </p>
                          {part.notes && (
