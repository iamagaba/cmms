          ...
          <div className="py-3">
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                {customerType && (
                  <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium border ${customerType === 'WATU' ? 'bg-purple-50 text-purple-700 border-purple-200' :
            ...
            <div className="flex items-start gap-3 mb-3">
              {vehicle?.image_url ? (
                ...
                <div className="w-20 h-20 bg-gray-100 rounded-md border border-gray-200 flex items-center justify-center flex-shrink-0">
                  <HugeiconsIcon icon={Motorbike01Icon} size={40} className="text-gray-300" />
                </div>
              )}
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                ...
                {vehicle?.status && (
                  <span className={`inline-flex items-center gap-1 px-2 py-1 rounded text-xs font-medium border ${vehicle.status === 'Normal' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' :
...
const WarrantyStatus: React.FC<{ vehicle?: Vehicle | null }> = ({ vehicle }) => {
  ...
  return (
    <div className={`mt-2 px-2 py-1.5 rounded border ${isExpired
      ...
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-1.5 flex-1 min-w-0">
          ...
          />
          <div className="flex-1 min-w-0">
            <p className={`text-xs font-medium ${isExpired ? 'text-red-700' : isExpiringSoon ? 'text-amber-700' : 'text-emerald-700'
              }`}>
        ...
        <div className="text-right flex-shrink-0">
          <p className={`text-sm font-bold ${isExpired ? 'text-red-700' : isExpiringSoon ? 'text-amber-700' : 'text-emerald-700'
            }`}>
            {formatDaysRemaining()}
          </p>
        </div>
