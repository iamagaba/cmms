-- Quick fix for the custody sync trigger
...
-- Fix existing records that should be 'In Repair'
UPDATE vehicles 
...
WHERE id IN (
    ...
    AND wo.vehicle_id NOT IN (
        SELECT v.id FROM vehicles v WHERE v.status = 'In Repair'
    )
);