      ...
      <div className={`${isMobileView ? 'w-full' : 'w-80'} ${isMobileView && selectedChat ? 'hidden' : 'flex'} bg-white dark:bg-gray-900 border-r border-gray-200 dark:border-gray-800 flex-col`}>
        ...
        <div className={`${spacing.card} border-b border-gray-200 dark:border-gray-800`}>
          <div className="flex items-center justify-between">
            <h1 className={`${spacing.text.heading} font-semibold text-gray-900 dark:text-gray-100`}>WhatsApp Chats</h1>
            <div className={`flex items-center ${spacing.gap}`}>
              <button className={`${isCompact ? 'p-1.5' : 'p-2'} hover:bg-gray-100 dark:hover:bg-gray-800 ${spacing.rounded} transition-colors`}>
                <HugeiconsIcon icon={Search01Icon} size={spacing.icon.md} className="text-gray-500 dark:text-gray-400" />
              </button>
              <button className={`${isCompact ? 'p-1.5' : 'p-2'} hover:bg-gray-100 dark:hover:bg-gray-800 ${spacing.rounded} transition-colors`}>
                <HugeiconsIcon icon={MoreVerticalIcon} size={spacing.icon.md} className="text-gray-500 dark:text-gray-400" />
              </button>
            </div>
          </div>
        </div>
        ...
        {/* Search Bar */}
        <div className={`${spacing.card} border-b border-gray-100 dark:border-gray-800 ${spacing.section}`}>
          <div className="relative">
            <HugeiconsIcon icon={Search01Icon} size={spacing.icon.sm} className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400" />
            <input
              type="text"
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              placeholder="Search chats..."
              className={`w-full pl-10 pr-4 ${spacing.input} border border-gray-200 dark:border-gray-700 ${spacing.roundedLg} bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent`}
            />
          </div>
          ...
          <div className={`flex flex-wrap ${spacing.gap}`}>
            {allTags.slice(0, 4).map((tag) => (
              <button
                ...
                className={`px-2 py-1 ${spacing.text.caption} ${spacing.rounded} transition-colors border ${selectedTags.includes(tag)
            ...
            {selectedTags.length > 0 && (
              <button
                ...
                className={`px-2 py-1 ${spacing.text.caption} text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/30 ${spacing.rounded} border border-red-200 dark:border-red-800`}
              >
                Clear
              </button>
            )}
