    ...
    >
      <Form form={form} layout="vertical" name="work_order_form">
        <Row gutter={16}>
          ...
          <Col span={12}><Form.Item name="locationId" label="Location" rules={[{ required: true }]}><Select>{locations.map(l => <Option key={l.id} value={l.id}>{l.name}</Option>)}</Select></Form.Item></Col>
          ...
          <Col span={24}><Form.Item name="slaDue" label="SLA Due Date" rules={[{ required: true }]}><DatePicker style={{ width: '100%' }} /></Form.Item></Col>
        </Row>
      </Form>
    </Modal>
  );
